...
1. Confirm containers are running with `docker compose -f deploy/docker/docker-compose.yml ps`.
...
- `docker compose -f deploy/docker/docker-compose.yml logs -f api-server`
- `docker compose -f deploy/docker/docker-compose.yml logs -f prometheus`

## Config Reloads

- After editing Nginx config, run `docker compose -f deploy/docker/docker-compose.yml restart nginx`.
