//! Funding-rate carry alpha sleeve (plan §3 — multi-factor alpha overlay).
//!
//! Funding-carry signal: when funding rate is negative, longs are paid → long
//! bias; when positive, shorts are paid → short bias. This is an independent
//! alpha source from M1. Tested as a standalone sleeve and combined with M1.
...
fn main() -> Result<()> {
    let metrics_dir = std::env::args().nth(1).ok_or_else(|| anyhow!("usage: r23_funding_carry <metrics_dir> [budget] [fo_pct]"))?;
    ...
    println!("=== FUNDING-CARRY SLEEVE (standalone) ===");
    ...
    // Load funding rates per symbol from the funding db.
    let fund_con = Connection::open_with_flags("data/funding_rates.db", rusqlite::OpenFlags::SQLITE_OPEN_READ_ONLY)?;
    let mut funding_by_sym: BTreeMap<String, BTreeMap<i64, f64>> = BTreeMap::new();
    for sym in SYMBOLS {
        let mut stmt = fund_con.prepare("SELECT funding_time, funding_rate FROM funding_rates WHERE symbol = ?1 ORDER BY funding_time")?;
        let rows = stmt.query_map([sym], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, f64>(1)?)))?;
        let mut m: BTreeMap<i64, f64> = BTreeMap::new();
        for row in rows { let (t, fr) = row?; m.insert(t, fr); }
        funding_by_sym.insert(sym.to_string(), m);
    }
    drop(fund_con);

    // common window from metrics
    ...
    // --- Funding-carry standalone sleeve ---
    // Signal: at each 5m bar, look up the most recent funding rate. If funding <
    // -threshold (very negative), long. If funding > +threshold, short.
    let fund_thresh = 0.0001; // 1 bps funding rate threshold
    ...
    for sym in SYMBOLS {
        ...
        let funding = funding_by_sym.get(*sym).cloned().unwrap_or_default();
        let signal = FundingSignal { funding, threshold: fund_thresh };
        ...
        println!("  {sym}: end_eq={:.1} trades={}", end_eq, res.metrics.trade_count);
    }
    ...
    println!("\n=== FUNDING-CARRY STANDALONE ===");
    println!("ann={:.2}% dd={:.2}% sharpe={:.4} skew={:.3} kurt={:.3} DSR={:.4} PBO={:.4}", ann, max_dd, ann_sharpe, st.skew, st.kurtosis, dsr, pbo);
    Ok(())
}

struct FundingSignal { funding: BTreeMap<i64, f64>, threshold: f64 }
impl SignalSource for FundingSignal {
    fn gate(&self, _i: usize, ts: i64, _price: f64) -> SignalGate {
        // find the most recent funding rate at or before ts
        let recent = self.funding.range(..=ts).next_back().map(|(_, &fr)| fr);
        match recent {
            Some(fr) if fr <= -self.threshold => SignalGate { long_fo: true, short_fo: false, so_allowed: true, force_abort: false },
            Some(fr) if fr >= self.threshold => SignalGate { long_fo: false, short_fo: true, so_allowed: true, force_abort: false },
