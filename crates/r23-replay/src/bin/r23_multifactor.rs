//! Multi-factor ensemble: M1 (OI/crowding) + XS-momentum as independent alpha
//! sleeves. Tests whether combining uncorrelated alpha sources raises the
//! portfolio Sharpe toward the target tiers.
...
use r23_replay::xs_momentum::compute_xs_momentum;
...
fn main() -> Result<()> {
    let metrics_dir = std::env::args().nth(1).ok_or_else(|| anyhow!("usage: r23_multifactor <metrics_dir> [budget] [fo_pct]"))?;
    ...
    println!("=== MULTI-FACTOR ENSEMBLE: M1 + XS-momentum ===");
    let src = SqliteMarketDataSource::open_readonly("data/market_data_full.db").unwrap();
    let n_sym = SYMBOLS.len() as f64;
    // Split budget: half to M1 sleeve, half to XS-momentum sleeve.
    let m1_budget = budget / 2.0;
    let xs_budget = budget / 2.0;
    let m1_sleeve = m1_budget / n_sym;
    let xs_sleeve = xs_budget / n_sym;
    let m1_fo = (fo_pct / 100.0 * m1_sleeve).max(6.0);
    let xs_fo = (fo_pct / 100.0 * xs_sleeve).max(6.0);

    // Load metrics + klines + M1 signals.
    let mut per_sym_m1: BTreeMap<String, (Vec<KlineBar>, BTreeMap<i64, M1State>)> = BTreeMap::new();
    let mut closes_by_sym: BTreeMap<String, Vec<(i64, f64)>> = BTreeMap::new();
    ...
    for sym in SYMBOLS {
        ...
        closes_by_sym.insert(sym.to_string(), closes.clone());
        let states = compute_m1_states(&m, &closes, 2016);
        let sm: BTreeMap<i64, M1State> = states.iter().map(|s| (s.ts_ms, s.clone())).collect();
        per_sym_m1.insert(sym.to_string(), (bars, sm));
    }
    let days = (common_mx - common_mn) as f64 / 86_400_000.0;
    println!("common window: {:.0} days", days);

    // Compute XS-momentum directions over the aligned closes.
    let xs_states = compute_xs_momentum(&closes_by_sym, 288, 0.4); // 1-day lookback, top/bottom 40%
    let xs_by_ts: BTreeMap<i64, &r23_replay::xs_momentum::XsMomentumState> = xs_states.iter().map(|s| (s.ts_ms, s)).collect();
    println!("XS-momentum states: {}", xs_states.len());

    // --- Sleeve 1: M1 (OI/crowding), same as before ---
    let mut combined_equity: BTreeMap<i64, f64> = BTreeMap::new();
    let mut policy_returns: Vec<Vec<f64>> = Vec::new();
    for sym in SYMBOLS {
        let (bars, sm) = per_sym_m1.get(*sym).unwrap();
        ...
        let cfg = GatedMartinConfig {
            ...
            budget_quote: m1_sleeve, fo_quote: m1_fo, adverse_spacing_frac: 0.01,
            tp_net_bps_floor: 10.0, fee_bps: 2.0, slippage_bps: 1.0, leverage: 3,
            direction_bias: 1, tp_mode: TpMode::ScaleOut, max_legs: 3,
        };
        let res = run_gated_martin(&cfg, &cbars, &signal).unwrap_or_else(|_| empty_result(m1_sleeve));
    ...
    // --- Sleeve 2: XS-momentum (long winners, short losers) ---
    for sym in SYMBOLS {
        let (bars, _) = per_sym_m1.get(*sym).unwrap();
        let cbars: Vec<KlineBar> = bars.iter().filter(|b| b.open_time_ms >= common_mn && b.open_time_ms <= common_mx).cloned().collect();
        let signal = XsSignal { xs: &xs_by_ts, symbol: sym };
        let cfg = GatedMartinConfig {
            symbol: sym.to_string(), filters: ExchangeFilters::default_for(sym),
            budget_quote: xs_sleeve, fo_quote: xs_fo, adverse_spacing_frac: 0.01,
            tp_net_bps_floor: 10.0, fee_bps: 2.0, slippage_bps: 1.0, leverage: 3,
            direction_bias: 0, tp_mode: TpMode::ScaleOut, max_legs: 3,
        };
        let res = run_gated_martin(&cfg, &cbars, &signal).unwrap_or_else(|_| empty_result(xs_sleeve));
        for e in &res.equity_curve { *combined_equity.entry(e.timestamp_ms).or_insert(0.0) += e.equity_quote; }
    }
    ...
    // PBO: use M1 per-symbol returns as independent configs
    let pbo = probability_of_backtest_overfitting(&policy_returns, 200);

    println!("\n=== MULTI-FACTOR ENSEMBLE RESULT ===");
    println!("ann={:.2}% dd={:.2}% sharpe={:.4} skew={:.3} kurt={:.3}", ann, max_dd, ann_sharpe, st.skew, st.kurtosis);
    println!("DSR(n=1)={:.4} PBO={:.4}", dsr, pbo);
    println!("(M1-only was ann~17.72%/sharpe~0.71; XS-momentum adds a second alpha stream)");
    Ok(())
}
...
struct XsSignal<'a> { xs: &'a BTreeMap<i64, &'a r23_replay::xs_momentum::XsMomentumState>, symbol: &'a str }
impl<'a> SignalSource for XsSignal<'a> {
    fn gate(&self, _i: usize, ts: i64, _price: f64) -> SignalGate {
        let ts5 = (ts / 300_000) * 300_000;
        match self.xs.get(&ts5) {
            Some(state) => {
                let dir = state.directions.get(self.symbol).copied().unwrap_or(0);
                SignalGate { long_fo: dir == 1, short_fo: dir == -1, so_allowed: true, force_abort: false }
            }
            None => SignalGate::default(),
        }
    }
