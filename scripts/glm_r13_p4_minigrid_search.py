#!/usr/bin/env python3
"""GLM Round 13 Task P4: Native Minigrid 512 Sobol Search."""
...
def apply_minigrid(cfg, levels, spacing, frac_num, frac_den, min_profit, max_active):
    new_cfg = copy.deepcopy(cfg)
    for s in new_cfg["portfolio_config"]["strategies"]:
        rl = s.get("risk_limits", {})
        rl["dca_minigrid"] = {"levels_per_band": levels, "spacing_bps": spacing,
...
def evaluate_one(spec):
    ...
    full = replay(tmp_path, 4999, FULL_START, FULL_END, f"r13p4_{label}")
    seg = {}
    for nm, s, e in FULL_SEGMENTS: seg[nm] = replay(tmp_path, 4999, s, e, f"r13p4_{label}_{nm}")
...
def main():
    ...
    specs = []
    # Grid: levels × spacing × fraction × profit × max_active = ~512
            ...
            for frac_num, frac_den in [(1,8),(1,6),(1,4)]:
                for min_profit in [10,20,35,55]:
                    for max_active in [1,2,3]:
                        label = f"l{levels}_s{spacing}_f{frac_num}of{frac_den}_p{min_profit}_a{max_active}"
                        cfg = apply_minigrid(base, levels, spacing, frac_num, frac_den, min_profit, max_active)
                        specs.append((label, cfg, f"/tmp/r13p4_{len(specs):04d}.json"))
                        if len(specs) >= 512: break
                    if len(specs) >= 512: break
                if len(specs) >= 512: break
            if len(specs) >= 512: break
        if len(specs) >= 512: break
    ...
    el = time.time() - t0
    print(f"\n[r13P4] wrote {args.out}: {len(valid)} eval, {len(targets)} targets in {el:.0f}s")
    print(f"\n=== TOP 10 by ann ===")
    for v in valid[:10]:
        fm = v["full_metrics"]
