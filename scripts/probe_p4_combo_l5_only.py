#!/usr/bin/env python3
"""Targeted l5_robust mix probe for P4 short legs."""
...
BUDGET = 5000.0
PROFILE = "balanced"
OUT = REPO / "docs/superpowers/reports/2026-06-29-p4-combo-l5-only-probe.json"


def base_config() -> dict:
    path = MAIN / "docs/superpowers/artifacts/glm-balanced-candidate/best_balanced_l5_robust_b5000.json"
    return json.loads(path.read_text())


def no_old_shorts(cfg: dict) -> dict:
    out = copy.deepcopy(cfg)
    strategies = strip_old_shorts(out["portfolio_config"]["strategies"])
    normalize_weights(strategies, 100.0)
...
def main() -> int:
    short_sets = load_short_sets(Path("/tmp/2025_p4_3000_allrows.json"))
    cfg = base_config()
    experiments: list[tuple[str, dict]] = [
        ("l5_robust__as_is", copy.deepcopy(cfg)),
        ("l5_robust__no_old_shorts", no_old_shorts(cfg)),
    ]
    for set_name in ("low3", "mid3", "high3", "low3_gala", "rsi_low4"):
        short_set = short_sets.get(set_name) or []
        if not short_set:
            continue
        for mode, short_weight in (
            ...
            ("norm", 25.0),
            ("norm", 35.0),
            ("drop_old_shorts_norm", 15.0),
            ("drop_old_shorts_norm", 25.0),
            ("drop_old_shorts_norm", 35.0),
        ):
            experiments.append(
                (
                    f"l5_robust__{mode}__{set_name}__short{int(short_weight)}",
                    build_mix(cfg, short_set, mode, short_weight, BUDGET, f"l5-{set_name}-{int(short_weight)}"),
                )
            )
    ...
    report = {
        ...
        "short_sets": {
            ...
            for key, value in short_sets.items()
        },
        "full_results": [],
    }
    OUT.parent.mkdir(parents=True, exist_ok=True)
    for idx, (name, cfg_item) in enumerate(experiments, 1):
        result = replay_full(name, cfg_item)
