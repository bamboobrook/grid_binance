#!/usr/bin/env python3
"""GLM Round 2 Direction F: Recovery-Based Re-Entry — full grid with 5-seg validation.

Tests equity-reclaim re-entry after portfolio stop. The portfolio equity stop
fires on DD, but instead of waiting the full calendar cooldown, re-enter when
equity recovers a fraction of the stopped drawdown.

Builds on r2-B-best-4of5 (partial TP + conditional SO) and adds portfolio DD
stop with equity-reclaim re-entry.

Grid: DD stop (12,16,20,25,30) × reclaim fraction (0.25,0.50,0.75,none) ×
cooldown (6,12,24h) on the B-best structure.

Usage:
  python3 scripts/glm_r2_dirF_search.py \
      --out docs/superpowers/artifacts/glm-martingale-core-round2/r2-F-full-grid.json
"""
import argparse
import json
import os
...
def run_replay(config, budget, s, e, pid):
    p = f"/tmp/r2Ff_{os.getpid()}_{pid}.json"
...
def build(dd_stop, reclaim_frac, cooldown_hours):
    longs = ["BNBUSDT", "TRXUSDT", "BCHUSDT"]
    shorts = ["AAVEUSDT", "SOLUSDT", "DOTUSDT"]
    strat = [mk(f"L{i}-{s}", s, "long", [x.format(S=s) for x in LONG_STRICT], 13.3)
             for i, s in enumerate(longs)]
    strat += [mk(f"S{i}-{s}", s, "short", [x.format(S=s) for x in SHORT_MID], 8.0)
              for i, s in enumerate(shorts)]
    rl = {"max_global_budget_quote": "5000"}
    if dd_stop > 0:
        rl["portfolio_equity_stop_pct"] = dd_stop
        rl["portfolio_stop_cooldown_hours"] = cooldown_hours
        if reclaim_frac is not None:
            rl["reentry_equity_reclaim_fraction"] = reclaim_frac
...
def main():
    ...
    dd_stops = [0, 16, 20, 25, 30]
    reclaims = [None, 0.25, 0.50, 0.75]
    cooldowns = [6, 12, 24]
    jobs = []
    for dd in dd_stops:
        for rc in reclaims:
            for cd in cooldowns:
                if dd == 0 and (rc is not None or cd != 12):
                    continue  # no stop => no reclaim/cooldown
                cfg = build(dd, rc, cd)
                rcl = "none" if rc is None else str(rc)
                lbl = f"dd{dd}-rc{rcl}-cd{cd}"
                jobs.append((cfg, lbl))

    print(f"[r2F-full] {len(jobs)} candidates x 6 replays, {args.workers} workers", flush=True)
    t0 = time.time()
    results = []
    ...
    for v in results:
        ...
        v["frontier_improvement"] = (
            (ann > 18.0 or dd < 19.0 or v["positive_segments"] >= 4)
    ...
    print(f"\n[r2F-full] wrote {args.out}: {len(results)} results, "
