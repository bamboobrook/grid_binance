#!/usr/bin/env python3
"""GLM Round 8 Task P4: Cost-Cover TP and Rescue Exit search.

Tests compressed TP after late legs + max cycle age exit. Uses existing engine
features (partial TP, max_cycle_age_hours, no_progress_exit) on R7 bases.
"""
...
def run_replay(config, budget, s, e, pid):
    p = f"/tmp/r8P4_{os.getpid()}_{pid}.json"
...
def apply_exit(base, max_age_h, no_progress_h, no_progress_mfe, be_after):
    cfg = copy.deepcopy(base)
    for s in cfg["strategies"]:
        rl = s.get("risk_limits", {})
        if max_age_h:
            rl["max_cycle_age_hours"] = max_age_h
        if no_progress_h:
            rl["no_progress_exit_hours"] = no_progress_h
            rl["no_progress_mfe_bps"] = no_progress_mfe
        s["risk_limits"] = rl
        # Adjust TP breakeven
        if "partial" in s.get("take_profit", {}):
            s["take_profit"]["partial"]["breakeven_after_stage"] = be_after
    return cfg
...
def main():
    ...
    # Grid per plan
    max_ages = [None, 72, 168, 336]  # hours
    no_progress = [(None, None), (72, 200), (168, 200), (168, 400)]
    be_afters = [1, 2]
    ...
    for bname, bpath in BASES.items():
        ...
        for ma in max_ages:
            for np_h, np_mfe in no_progress:
                for bea in be_afters:
                    if ma is None and np_h is None and bea == 1: continue  # skip baseline duplicate
                    cfg = apply_exit(base, ma, np_h, np_mfe, bea)
                    tag = f"ma{ma}np{np_h}_{np_mfe}be{bea}"
                    jobs.append((cfg, f"{bname}_{tag}"))

    print(f"[r8P4] {len(jobs)} candidates x 6 replays, {args.workers} workers", flush=True)
    t0 = time.time()
    results = []
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if done % 20 == 0 or (ann > 45 and dd <= 22):
                print(f"  [{done}/{len(jobs)}] {rec.get('label','?'):30s} ann={ann} dd={dd} pos={rec.get('positive_segments')}/5", flush=True)
    ...
    print(f"\n[r8P4] wrote {args.out}: {len(results)} results, {nt} target hits in {time.time()-t0:.0f}s")
    print("\n=== TOP 10 ===")
    for v in results[:10]:
        if "error" in v: continue
        fm = v.get("full_metrics") or {}
        print(f"  {v['label']:30s} ann={fm.get('ann'):7.1f} dd={fm.get('dd'):6.1f} pos={v['positive_segments']}/5")
