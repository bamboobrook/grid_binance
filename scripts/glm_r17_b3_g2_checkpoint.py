#!/usr/bin/env python3
"""B3 G2 anchored nested WFO — uses REAL G1 Pareto survivors (with stored params).
G2 strict gate: median ann>=30%, worst block DD<=35%, >=3/5 positive, no breach.
Then 4 anchored WFO folds on survivors."""
...
def build(track, p):
    symbols = TRACK_SYMS[track]
    n=len(symbols); wt=round(100.0/(2*n),4)
    rl={"htf_regime_gate_enabled":True,
        "r17_router":{"trend_horizon_4h":int(p["th"]),"breadth_threshold":0.70,"enter_persistence":3,"exit_persistence":2,"minimum_dwell_hours":24,"range_displacement_z":1.25,"shock_downside_q":0.90,"cusum_sigma":4.0,"shock_cooldown_hours":24},
        "r17_hazard":{"half_life_window_h":168,"deadline_half_lives":int(p["dl"]),"deadline_cap_h":72,"after_deadline":"freeze_so","reserve_next_legs":2}}
    strats=[]
    for sym in symbols:
        strats.append(_s(sym,"long",p["fo"],p["mult"],p["legs"],p["sp"],p["tp"],p["lev"],wt,rl))
        strats.append(_s(sym,"short",p["fo"],round(p["mult"]*0.85,2),max(3,p["legs"]-1),p["sp"]+50,p["tp"],p["lev"],wt,rl))
...
def main():
    ...
    survivors=g1["pareto_survivors"][:8]  # G2 input <=8
    g2_results=[]; g2_survivors=[]
    for i,s in enumerate(survivors):
        track=s["track"]; p=s["params"]
        cfg=build(track,p)
        lb=f"r17_g2_{track}_{i:02d}_fo{p['fo']}_m{p['mult']}_l{p['legs']}_s{p['sp']}_t{p['tp']}_lv{p['lev']}_th{p['th']}_dl{p['dl']}"
        ...
        g2_results.append({"track":track,"params":p,"full_ann":full.get("ann"),"full_dd":full.get("dd"),
                           "seg_pos":pos,"worst_seg_dd":worst_dd,"median_ann":med,"any_breach":any_breach,"strict_g2_pass":strict})
        if strict: g2_survivors.append({"track":track,"params":p})
        print(f"G2 {track} fo{p['fo']} m{p['mult']}: full ann={full.get('ann')} dd={full.get('dd')} pos={pos} worst_dd={worst_dd:.1f} med={med:.1f} breach={any_breach} STRICT={strict}",flush=True)
    # G3 nested WFO
    folds_out=[]; finalists=[]
    if g2_survivors:
        sel=g2_survivors[0]; cfg=build(sel["track"],sel["params"])
    ...
    print(f"\nB3 done: G2 survivors={len(g2_survivors)} folds={len(folds_out)} finalists={len(finalists)}")
