#!/usr/bin/env python3
"""GLM Martingale Core — DD optimization on the high-ann structure.

The ATR/efficiency search found that strict-gate + 600bps TP + high multiplier
produces ann~73% but DD~45%. This script sweeps TIGHT DD stops (8-18%) on that
exact high-ann structure to find the ann/DD sweet spot (target ann>50, DD<=30).

Also varies the short basket size and weights to find the best combo.

Usage:
  python3 scripts/glm_highann_dd_optimize.py \
      --out docs/superpowers/artifacts/glm-martingale-core/highann-dd-opt.json
"""
import argparse
import json
import os
...
def run_replay(config, budget, s, e, profile, pid):
    p = f"/tmp/glm_hd_{os.getpid()}_{pid}.json"
...
def mk(sid, symbol, direction, gates, p, w, tp_bps):
...
def build(longs, shorts, lw, sw, tp_long, tp_short, mult_l, mult_s, legs_l, legs_s,
          dd_stop, cd_hours, adx_skip):
    lp = dict(leverage=10, multiplier=mult_l, max_legs=legs_l, step_bps=150,
              sl_bps=5000, cooldown=21600, first_q=35.0, adx_skip=adx_skip)
    sp = dict(leverage=10, multiplier=mult_s, max_legs=legs_s, step_bps=180,
              sl_bps=4000, cooldown=21600, first_q=30.0, adx_skip=adx_skip)
    strat = []
    for i, s in enumerate(longs):
        strat.append(mk(f"hd-L{i}-{s}", s, "long",
                        [x.format(S=s) for x in LONG_STRICT], lp, lw, tp_long))
    for i, s in enumerate(shorts):
        strat.append(mk(f"hd-S{i}-{s}", s, "short",
                        [x.format(S=s) for x in SHORT_STRICT], sp, sw, tp_short))
...
def main():
    ...
    # Tight DD stops — KEY to reduce 45% DD
    dd_stops = [(8, 12), (8, 24), (10, 12), (10, 24), (12, 12), (12, 24),
                (14, 24)]
                        ...
                        for lw, sw in weight_sets:
                            for adx in adx_skips:
                                cfg = build(ls, ss, lw, sw, tpl, tps, ml, ms, 9, 8,
                                            dd, cd, adx)
                                lbl = (f"L{len(ls)}S{len(ss)}-t{tpl}{tps}"
                                       f"-m{ml}{ms}-d{dd}c{cd}a{adx}-{lw}w{sw}")
                                jobs.append((cfg, lbl))

    print(f"[glm-hd] {len(jobs)} candidates x 6 replays, {args.workers} workers",
          flush=True)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if done % 30 == 0 or (fm.get("ann") and fm["ann"] > 50
                                  and (fm.get("dd") or 999) <= 30):
    ...
    print(f"\n[glm-hd] wrote {args.out}: {len(results)} results, {n_pass} agg-passes "
