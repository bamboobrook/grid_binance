...
def build_variant(
    ...
    mode = rng.choice(["dd_cut", "dd_cut", "age_exit", "age_exit", "dd_income", "robust_mix", "short_boost"])
    if profile == "conservative":
        mode = rng.choice(["dd_cut", "robust_mix", "age_exit", "dd_income"])
    if profile == "aggressive":
        mode = rng.choice(["short_boost", "short_boost", "dd_income", "age_exit", "robust_mix"])
    ...
    age = rng.choice([24.0, 48.0, 72.0, 96.0, 168.0])
...
def passes(args: argparse.Namespace, item: dict[str, Any]) -> bool:
    ...
    if seg["max_segment_dd"] is not None and seg["max_segment_dd"] > t["max_seg_dd"]:
        return False
    h1_limit = {"conservative": 0.35, "balanced": 0.45, "aggressive": 0.55}[args.profile]
    if seg["h1_contribution_ratio"] > h1_limit:
        return False
    if args.profile in ("balanced", "aggressive"):
        vals = seg.get("segment_returns") or []
        if len(vals) >= 5 and sum(vals[2:]) < 0:
            return False
    return True
...
def main() -> int:
    ...
    def score(item: dict[str, Any]) -> float:
        ...
        ret = float(full.get("ret") or 0.0)
        cap = float(full.get("cap") or 0.0)
        trades = float(full.get("trades") or 0.0)
        t = TARGETS[args.profile]
        blocked = int(full.get("blocked") or 0)
        dd_penalty = 5.5 * max(0.0, dd - t["dd"])
        target_penalty = 0.6 * max(0.0, t["ann"] - ann)
        blocked_penalty = 3.0 * blocked
        churn_penalty = 0.0008 * max(0.0, trades - 9000.0)
        cap_bonus = min(cap / max(args.budget, 1.0), 1.0) * 2.0
        return ann - dd_penalty - target_penalty - blocked_penalty - churn_penalty + cap_bonus + 0.01 * ret
