...
import { Activity, History, ShieldCheck, Wallet, Zap } from "lucide-react";
  ...
  return (
    <div className="flex flex-col space-y-4 max-w-[1600px] mx-auto h-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {metrics.map((metric) => (
          <div key={metric.label} className="bg-card border border-border/60 rounded-sm p-4 flex flex-col justify-center">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-muted text-muted-foreground text-[10px] uppercase tracking-wider">
                  <tr>
                    <th className="px-4 py-2 font-medium">{copy(locale, "交易对", "Pair")}</th>
                    <th className="px-4 py-2 font-medium text-right">PnL</th>
                    <th className="px-4 py-2 font-medium text-right">{copy(locale, "状态", "Status")}</th>
                  </tr>
                </thead>
                      ...
                      <tr key={index} className="hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <span className={`px-1.5 py-0.5 rounded-sm text-[10px] font-bold ${isPositive ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>
                            {isPositive ? copy(locale, "已止盈", "Closed") : copy(locale, "追踪中", "Trailing")}
                          </span>
                        </td>
                      </tr>
                    );
                  ...
                  {(!analytics?.fills || analytics.fills.length === 0) && (
                    <tr>
                      <td colSpan={3} className="px-4 py-8 text-center text-xs text-muted-foreground">
                        {copy(locale, "暂无最近成交，先启动一个策略看看。", "No recent deals. Start a bot to see activity.")}
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-foreground font-semibold">{copy(locale, "币安现货", "Binance Spot")}</span>
                </div>
                <span className="text-[10px] bg-secondary text-muted-foreground px-1.5 py-0.5 rounded-sm">{copy(locale, "已连接", "Connected")}</span>
              </div>
              ...
              <div className="space-y-2">
                {analytics?.wallets[0]
                  ...
                  : <p className="text-xs text-muted-foreground text-center py-2">{copy(locale, "暂无余额数据", "No balance data")}</p>}
              </div>
            </div>
          </Card>
          ...
          <Card className="bg-card border-border shadow-none">
            <div className="bg-secondary/30 px-4 py-2.5 border-b border-border">
              <span className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
                ...
                {copy(locale, "系统状态", "System Status")}
              </span>
            </div>
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">{copy(locale, "交易引擎", "Trading Engine")}</span>
                <span className="text-emerald-500 font-semibold">{copy(locale, "运行正常", "Operational")}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">{copy(locale, "行情服务", "Market Data")}</span>
                <span className="text-emerald-500 font-semibold">{copy(locale, "同步中", "Syncing")}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">{copy(locale, "延迟", "Latency")}</span>
