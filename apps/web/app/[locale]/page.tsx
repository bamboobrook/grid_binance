...
export default async function HomePage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-primary/30">
      {/* Top Navigation */}
      <nav className="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto w-full z-10 border-b border-border/50 bg-background/80 backdrop-blur-md sticky top-0">
        ...
        <div className="flex items-center gap-5">
          <ThemeToggle />
          <LocaleToggle />
          <Link href={`/${locale}/login`} className="text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors hidden sm:block">
            {t('viewDemo')}
          </Link>
      ...
      <header className="flex-1 flex flex-col items-center justify-center text-center px-4 py-24 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-card border border-border text-foreground text-xs font-bold mb-10 shadow-sm">
            <Activity className="w-3.5 h-3.5 text-primary" />
            {t('advancedGrid') || "Advanced Grid Trading Bots"}
          </div>
          
          <h1 className="text-6xl md:text-8xl font-extrabold text-foreground tracking-tight leading-[1.05] max-w-5xl mx-auto mb-8 drop-shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 w-full sm:w-auto">
            ...
            <Link href={`/${locale}/login`} className="w-full sm:w-auto">
              <Button className="w-full sm:w-auto px-10 h-16 text-lg rounded-xl font-bold border border-border bg-card text-foreground hover:bg-slate-800 hover:border-border transition-all">
                {t('viewDemo')}
              </Button>
            </Link>
          </div>
      ...
      {/* Feature Highlights */}
      <section className="py-28 bg-background border-t border-border relative z-10">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="space-y-5 p-8 rounded-2xl bg-card border border-border hover:border-border transition-colors">
            <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20">
              <LineChart className="w-6 h-6 text-indigo-400" />
            </div>
            <h3 className="text-2xl font-bold text-foreground">{t('features.smartTrade.title')}</h3>
            <p className="text-muted-foreground leading-relaxed text-base">
              {t('features.smartTrade.desc')}
            </p>
          </div>
          <div className="space-y-5 p-8 rounded-2xl bg-card border border-border hover:border-border transition-colors relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-bl-full pointer-events-none" />
            <div className="w-12 h-12 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20">
              <Zap className="w-6 h-6 text-emerald-400" />
            </div>
            <h3 className="text-2xl font-bold text-foreground">{t('features.dcaBots.title')}</h3>
            <p className="text-muted-foreground leading-relaxed text-base">
          ...
          <div className="space-y-5 p-8 rounded-2xl bg-card border border-border hover:border-border transition-colors">
            ...
            <h3 className="text-2xl font-bold text-foreground">{t('features.riskManagement.title')}</h3>
            <p className="text-muted-foreground leading-relaxed text-base">
