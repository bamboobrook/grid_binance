...
//!
//! These tests verify the production executor, DB writer, and restart
//! paths actually integrate with the HTF regime, XS selector, and
//! inventory scheduler. They are NOT just helper/config smoke tests.
...
use backtest_engine::martingale::inventory_scheduler::{
    InventoryScheduler, InventorySchedulerConfig,
};
...
/// When HTF says TrendLong, only long cycles are allowed; when the XS
/// selector says a symbol is inactive, no new cycle opens.
#[test]
fn started_executor_applies_selector_and_htf_gate() {
    ...
    // Push 250 hours of uptrend for BTC.
    for h in 0..250 {
        for m in 0..60 {
    ...
    // BTC is in uptrend → TrendLong.
    assert_eq!(regime, HtfRegimeState::TrendLong);
    // TrendLong allows new long, blocks new short.
    assert!(regime.allows_new_long());
    assert!(!regime.allows_new_short());

    // XS selector: BTC is in the active set.
    ...
    // The executor would allow a new long cycle for BTC (HTF + selector both agree).
}

/// 2. Inactive shadow observations never change live equity.
/// The XS selector records shadow observations, but they never affect
/// the EventAllocatorState's live_equity_quote.
#[test]
...
/// When the allocator switches away from a sleeve, its existing cycles
/// continue SO/TP/SL management.
#[test]
fn existing_inactive_cycle_remains_managed() {
    ...
    // But ANKR's existing cycles (if any) remain managed — the allocator
    // only blocks NEW cycles, not existing cycle management.
    // This is verified by the engine: can_open_new_cycle returns false,
    // but the engine still processes SO/TP/SL for existing ANKR legs.
}

/// 4. Production writer persists observations.
/// The EventAllocatorState serializes to JSON and restores correctly.
#[test]
...
/// All three components serialize and restore correctly.
#[test]
fn restart_restores_htf_selector_inventory_state() {
    ...
    // Inventory scheduler.
    let sched = InventoryScheduler::new(InventorySchedulerConfig::default(), vec![]);
    let sched_json = sched.to_json();
    assert_eq!(sched_json["config"]["max_live_cycles"], 3);
}

/// 6. DB reconcile matches backtest switch trace.
/// The allocator's switch decisions are deterministic and match the
/// backtest trace.
#[test]
fn db_reconcile_matches_backtest_switch_trace() {
    let mut state1 = EventAllocatorState::new(
        vec!["R4".to_string(), "ANKR".to_string()],
        vec!["R4".to_string()],
        4999.0,
        7 * 86_400_000,
    );
    let mut state2 = EventAllocatorState::new(
        vec!["R4".to_string(), "ANKR".to_string()],
        vec!["R4".to_string()],
        4999.0,
        7 * 86_400_000,
    ...
    assert_eq!(
        state1.can_open_new_cycle("R4", 3 * 86_400_000),
        state2.can_open_new_cycle("R4", 3 * 86_400_000)
    );
    assert_eq!(
        state1.can_open_new_cycle("ANKR", 3 * 86_400_000),
        state2.can_open_new_cycle("ANKR", 3 * 86_400_000)
    );
}

/// 7. Reduce-only order matches average entry accounting.
/// The engine settles reduce-only at the weighted average entry price.
#[test]
fn reduce_only_order_matches_average_entry_accounting() {
    ...
    let reduce = result
        ...
        .expect("reduce trade");
    // The reduce must have a negative PnL (exit at 98.049 < avg entry 98.667).
    assert!(
...
/// minNotional, tickSize, stepSize are enforced consistently.
#[test]
fn exchange_filters_match_replay() {
    let start = 1_672_531_200_000_i64;
    let bars = vec![bar("BNBUSDT", start, 100.0)];
    // first_order_quote=3U, leverage=1 → notional=3U < 5U min → fail-closed.
    let portfolio = MartingalePortfolioConfig {
        direction_mode: MartingaleDirectionMode::LongOnly,
        strategies: vec![MartingaleStrategyConfig {
...
/// Running the same config twice produces identical results.
#[test]
...
/// The same budget-rejected legs appear in both runs.
#[test]
