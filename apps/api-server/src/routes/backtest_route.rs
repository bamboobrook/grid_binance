...
use crate::{
    ...
    services::{
        ...
        martingale_publish_service::{
            ConfirmStartPortfolioRequest, LivePortfolio, MartingalePublishService, PublishError,
            PublishIntentResponse, PublishPortfolioRequest, PublishPortfolioResponse,
        },
    },
    AppState,
};
...
async fn confirm_start_portfolio(
    ...
    headers: HeaderMap,
    Path(id): Path<String>,
    request: Option<Json<ConfirmStartPortfolioRequest>>,
    ...
    Ok(Json(service.confirm_start_portfolio(
        &session.email,
        &id,
