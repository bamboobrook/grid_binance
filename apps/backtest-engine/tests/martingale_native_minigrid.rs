...
use shared_domain::martingale::{MartingaleDcaMiniGridConfig, MartingaleDirection};

fn cfg(levels: u32, spacing: u32, num: u32, den: u32, profit: u32, max_active: u32) -> MartingaleDcaMiniGridConfig {
...
fn native_minigrid_after_safety_fill_reduces_inventory_only() {
    ...
    assert!(l0 > last_fill, "long minigrid level 0 ({}) must be above last_fill ({})", l0, last_fill);
    assert!((l0 - 100.20).abs() < 0.001, "level 0 price should be 100.20, got {}", l0);
    // Level 1: 100 * (1 + (20+30)/10000) = 100.50
    let l1 = mg.level_price(last_fill, 1, true);
    assert!(l1 > l0, "levels must increase for longs");
    assert!((l1 - 100.50).abs() < 0.001, "level 1 price should be 100.50, got {}", l1);
    // The close fraction reduces inventory (1/4 = 25%)
    let frac = mg.close_fraction();
    assert!((frac - 0.25).abs() < 1e-9, "close fraction should be 0.25, got {}", frac);
...
fn native_minigrid_never_opens_without_active_safety_leg() {
    ...
    assert!(long_level > last_fill, "long level must be above fill (reduce by selling)");
    assert!(short_level < last_fill, "short level must be below fill (reduce by buying)");
}
...
fn native_minigrid_respects_min_notional_and_close_fraction() {
    ...
    assert!(close_value >= 5.0, "close value {} should be >= 5 USDT (min notional)", close_value);
    ...
    assert!(small_close < 5.0, "close value {} should be < 5 USDT (would be skipped)", small_close);
}

#[test]
fn native_minigrid_validation_rejects_invalid_configs() {
    // levels_per_band out of range
    assert!(cfg(0, 30, 1, 4, 20, 2).validate().is_err(), "levels=0 should fail");
    assert!(cfg(6, 30, 1, 4, 20, 2).validate().is_err(), "levels=6 should fail");
    // spacing_bps out of range
    assert!(cfg(3, 5, 1, 4, 20, 2).validate().is_err(), "spacing=5 should fail");
    assert!(cfg(3, 200, 1, 4, 20, 2).validate().is_err(), "spacing=200 should fail");
    // close_fraction_num = 0
    assert!(cfg(3, 30, 0, 4, 20, 2).validate().is_err(), "num=0 should fail");
    // close_fraction_den < num
    assert!(cfg(3, 30, 4, 2, 20, 2).validate().is_err(), "den<num should fail");
    // min_profit_bps < 5
    assert!(cfg(3, 30, 1, 4, 3, 2).validate().is_err(), "profit=3 should fail");
    // max_active_levels out of range
    assert!(cfg(3, 30, 1, 4, 20, 0).validate().is_err(), "max_active=0 should fail");
    assert!(cfg(3, 30, 1, 4, 20, 6).validate().is_err(), "max_active=6 should fail");
}
...
fn native_minigrid_level_price_symmetry() {
    ...
    for idx in 0..3 {
        ...
        assert!((long_offset - short_offset).abs() < 1e-9,
            "level {} not symmetric: long_offset={}, short_offset={}", idx, long_offset, short_offset);
    }
}
