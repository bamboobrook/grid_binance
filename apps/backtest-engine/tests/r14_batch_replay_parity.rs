    ...
    for &sym in &symbols {
        for &dir in &directions {
            for &fo in &fos {
                ...
                configs.push((
                    format!("cfg_{:02}_{}_{}_fo{}_m{}_l{}_tp{}_lev{}", idx, sym, if dir == MartingaleDirection::Long { "L" } else { "S" }, fo, mult, legs, tp, lev),
...
fn batch_parallel_matches_single_for_20_configs() {
    ...
    let single_results: Vec<_> = configs
        ...
        .map(|(label, config)| {
            (
                label.clone(),
                batch.run_single(config, 4999.0, DEV_START, window_end).expect("single"),
            )
        })
        .collect();
    ...
    for (parallel, (label, single)) in parallel_results.iter().zip(single_results.iter()) {
        ...
        assert_eq!(parallel.trade_count, single.trade_count, "trade count mismatch for {}", label);
        assert_eq!(
            parallel.budget_blocked_legs, single.budget_blocked_legs,
            "blocked legs mismatch for {}", label
        );
    }
}
...
fn batch_replay_20_configs_deterministic_across_runs() {
    ...
    let configs: Vec<(String, MartingalePortfolioConfig)> = vec![
        ("det_01".to_string(), make_portfolio("BNBUSDT", MartingaleDirection::Long, 50, 2, 5, 100, 10)),
        ("det_02".to_string(), make_portfolio("BNBUSDT", MartingaleDirection::Short, 40, 3, 8, 200, 5)),
        ("det_03".to_string(), make_portfolio("BNBUSDT", MartingaleDirection::Long, 30, 2, 5, 100, 10)),
    ];
    ...
    for (a, b) in run1.iter().zip(run2.iter()) {
        ...
        assert!((a.annualized_return_pct - b.annualized_return_pct).abs() < 1e-9, "ann not deterministic for {}", a.config_label);
        assert!((a.max_drawdown_pct - b.max_drawdown_pct).abs() < 1e-9, "DD not deterministic for {}", a.config_label);
        assert_eq!(a.trade_count, b.trade_count, "trade count not deterministic for {}", a.config_label);
    }
}
...
fn batch_replay_missing_symbol_fails_closed() {
    ...
    assert!(result.is_err(), "missing symbol must fail-closed, not silently skip");
...
fn batch_replay_bad_db_path_rejected() {
    let result = BatchReplay::new("/nonexistent/path/market.db", "data/funding_rates_round12.db");
...
fn batch_replay_100_configs_benchmark() {
    ...
    let parallel_results = batch.run_configs_parallel(configs.clone(), 4999.0, DEV_START, window_end);
    let parallel_duration = start.elapsed();

    assert_eq!(parallel_results.len(), 100);
    // All results must be valid (no errors).
    for result in &parallel_results {
        assert!(result.error.is_none(), "config {} errored: {:?}", result.config_label, result.error);
    }
    ...
    for (label, config) in configs.iter().take(sample_size) {
        let _ = batch.run_single(config, 4999.0, DEV_START, window_end).expect("single");
    }
...
fn batch_replay_immutable_shared_data_no_corruption() {
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        assert_eq!(result.trade_count, first.trade_count, "result {} trade count differs", i);
    }
}
